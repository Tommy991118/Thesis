...
#Checks the node type = POI (school, region, station)
...
class DataUtils:
    #node_negative_distribution_temp is of the data_loader of only attribute values.
    def __init__(self, graph_file, is_all=False, node_negative_distribution_temp = None, test_indices = None):
        
        self.test_indices = test_indices
        #Loads in arrays or pickled objects. Here it loads in the given array file of the edges given as argument.
        with np.load(graph_file, allow_pickle=True) as loader:
            loader = dict(loader) #Creates a dictionary, this is a collection which is unordered, changeable and indexed of the graph file (edges two POIs).
            self.A = sp.csr_matrix((loader['adj_data'], loader['adj_indices'],
                               loader['adj_indptr']), shape=loader['adj_shape']) #Compressed sparse row matrix. Load in A data regarding indices, adjacency and indptr(?).
            #If the general attributes file is not empty
            if loader['attr_data1'].all() != None:
                print("Attributes Found.")
                #Compressed sparse row matrix to load in all the attributes of the different POIs that are present in the first graph file.
                #Some are going to be empty? Because the graph file is of all the edges between two POIs.
            ...
            #The .keys() returns all the variables present in the graph file, set as attribute "labels" for the current object.
            if 'labels' in loader.keys():
                self.labels = loader['labels']
            else:
                self.labels = None
            # "not is_all" is set as default. Checks also if validation edges are present in the graph file?
            ...
            #Create adjacency matrix, gives 1 if two nodes are adjacent and 0 otherwise.
            #Needs a scipy sparse matrix as input and takes in a graph type to create.
            self.g = nx.from_scipy_sparse_matrix(self.A, create_using=nx.DiGraph())
            if type(self.test_indices) != type(None): #If the type of the indices is not zero, remove all the incoming and outgoing edges of the test set.
                self.g.remove_edges_from(list(self.g.in_edges(test_indices)))
                self.g.remove_edges_from(list(self.g.out_edges(test_indices)))
            #Gather number of edges and nodes of the graph.
            self.num_of_nodes = self.g.number_of_nodes()
            self.num_of_edges = self.g.number_of_edges()
            #Gives a view of edge attributes, iterates over (u, v) or (u, v, d).
            self.edges_raw = self.g.edges(data=True)
            #Gather node and attribute value. It's set to true so it becomes the entire data dictionary.
            ...
            #Edge distribution is defined as a multivariate Gaussian distribution(?).
            ...
            #When optimizing O2 the gradient gets multiplied by the edge weight -> high weight variance -> explodient gradients => alias sampling as solution.
...
class AliasSampling:
    #SEE LAST PARAGRAPH PAGE 14 of 3.4 IN DAS ET AL.
    #Using edge sampling from an alias table to avoid exploding gradient.
