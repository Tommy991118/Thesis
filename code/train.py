...
#Stratified random sampling applied, 80% for unsupervised training of embedding model for GSNE and 20% for testing the embedding model.
...
def score_node_classification(features, z, features_test, labels_test, p_labeled=0.8, n_repeat=10, norm=False):
    ...
    #stores the MAEs and MSEs to potentially plot them for every prediction.
    trace = []
...
def main():
    #Can see all the arguments that are passed under "args".
    #We define these arguments ourselves.
    #Creates a user friendly command-line interface.
...
def train(args):
    #Train the model for the given arguments.
    ...
    #Load in files that represent the edged between POIs, only relevant ones are taken. So not all possible combinations of POIs.
    ...
    #Performs stratified random sampling based on the price, creates 20 bins/intervals based on which test and train test are sampled.
    train, tesst = train_test_split_sampled(df_price, 'price', 20)
    #Below selects of all rows column index zero which are the indices.
    train_indices = train[:, 0]
    test_indices = tesst[:, 0]
    #Saves an array to a txt file.
    #Save the indices in the embedding path, both for train and test indices.
    ...
    #Data loaders cooridinates which data provider to use and provides hooks for preprocessing and postp rocessing and storing data on your database if required.

    #Pass the graph files which are the edges between POIs (only relevant connections given).
    ...
    #X represents the node attributes of the different POIs. Ex: attributes of the school/train/region/house
    #oh to signify to not embed something whilst other POIs are being embedded.
    #If the given proximity is "oh" then return an identity matrix otherwise use the attributes of the POI.
    #The paper mentioned that they tested with leaving out some POIs, this is probably the way they did it.
    ...
    m = args.model #Why do we have to extract model from the args? It's set as default equal to GSNE. 
    name = m + '_' + args.name
    if 'gsne' == m:
        model = GSNE(args) #Here we create the actual GSNE
    else:
        raise Exception("Only gsne available")


    writer = tf.summary.FileWriter(tensorboard_path) #Allows to create a tensorflow summary filewriter for tensorboard.
    saver = tf.train.Saver() #Save and restore variables, these are the weights and biases.

    #Runs the tensorboard session?
    with tf.Session() as sess:
        #saver.restore(sess, model_path+"model_graduate_best.ckpt")
        writer.add_graph(sess.graph) #Graph is added to the tensorboard
        print('-------------------------- ' + m + ' --------------------------') #Going to print out "gsne"
        print('batches\tloss\tsampling time\ttraining_time\tdatetime') #Print out the loss

        tf.global_variables_initializer().run() #A variable op that holds the variable value/initializer that sets value variable to its initial value.
        ...
        for b in range(args.num_batches):
            if b%35000 == 0:
              clear_output() #At batch nr 35000 clear output.
            t1 = time.time()
            #CREATE DIFFERENT SAMPLER
            if b%5 < 0: #Ran everything once then, so it's logical to set it to 5. All the different data loaders.
