...
#Deals w/ sparse matrices
def sparse_feeder(M):
    M = sp.coo_matrix(M, dtype = np.float32) #Makes a sparse matrix in COOrdinate format. The first argument is a dense matrix.
    return np.vstack((M.row, M.col)).T, M.data, M.shape #Stack arrays in sequence vertically, row wise. Also returns the data of the matrix and the shape.

class GSNE:
  #Creates an object of the class. Self is used to access variables that belong to the class. It's a reference to the current instance of the class. 
  #Args are the following: Network G=(V,X,E,W) - with V=nodes, x=attributes of nodes, E = edges and w = weights on edges.
  #Further the other arguments are: proximity, number of negative samples N,  batch size b, total iterations T.
    def __init__(self, args):
        tf.set_random_seed(seed)
        #A sparsetensor is represented as three separate dense tensors: indices, values, and dense_shape
        #This is returned by the sparse_Feeder function. This way we create the matrices, these represent the POIs.
        #The X's are the attributes of the different nodes, if I'm not mistaken.
        # The * before the function call means: pack all positional arguments into a tuple.
        ...
        #Get rows and dimensions of the 4 POIs, so we know how many attributes they have and observations.
        self.N1, self.D1 = args.X1.shape
        self.N2, self.D2 = args.X2.shape
        self.N3, self.D3 = args.X3.shape
        self.N4, self.D4 = args.X4.shape

        #The dimension of embedding is also given, extract this from the args.
        #The embedding dimension has to be smaller than or equal to the amount of nodes and smaller than or equal to Dk (which is the dimension of attributes of the POI).
        #k refers to a single type of POI, which is mutually exclusive subset of nodes.
        ...
        #Create a GSNE model with the given arguments of self. It passes as argument the proximity, which is first or second order.
        #Further it will initalize the weights and biases for the four POIs for the given proximity (first order or both first and second order).
        #The result will be an initial model with weights for all the attributes of the different POIs. Is this the intermediate representation of a POI as shown in figure (d) page 8?
        self.__create_model(args.proximity)
        #I think we specify this ourselves to specify that we as a standard don't utilize a validaiton set, by setting the parameter of the argument to False.
        self.val_set = False
        
        #Global_step refers to the numbero f batches seen by the graph.
        ...
        # softmax loss: is identical to softmax layer followed by a multinomial loss layer.
        #Calculate the KL dissimilarity measure, this is done first and second order proximity. 
        self.energy = -self.energy_kl(self.u_i, self.u_j, args.proximity, self.node_type1, self.node_type2)
        #tf.reduce_mean does the same as np.mean() but for tensors.
        ...
        tf.summary.scalar("current_step",global_step) #Keeps indeed track of the current number of batches seen so far.
 ...
 '''
        ...
        self.merged_summary = tf.summary.merge_all() #Merges all summaries collected in the default graph.
        

    def __create_model(self, proximity):
      #Performs xavier initialization for the weights.
        w_init = tf.contrib.layers.xavier_initializer
        #w_init = tf.random_normal_initializer(mean=0.0, stddev=0.1, seed=None)
        #w_init = tf.keras.initializers.random_normal
        #Sizes is the amount of features/dimensions plus the hidden layer architecture of self? So that the size = variables+hidden nodes
        ...
        #Below we're going to initialize the four POIs: train, region, school and house.

        #feature 1
        TRAINABLE = True
        #Pushes a name scope which will make the name of all operations added within it have a prefix. Here it's "Train" as prefix.
        with tf.name_scope("Train"):
          #Loop through all the weights and biases.
          #Creates the weights and biases based on the size, this is will given to the shape of the new variable too.
          #This way we get a W and b which stores all the weights and biases, which are initialized w/ xavier.
          for i in range(1, len(sizes1)):
             with tf.name_scope("enc{}".format(i)):
              ...
              #Done for the first one, don't know why though.
              #The input must be a sparse tensor, which is the case. In the constructor function we can see that X1 is a sparse tensor.
              #X1 represents all the attributes of the POI which is then matmul to their weights added up to the bias.
              if i == 1:
                  encoded1 = tf.sparse_tensor_dense_matmul(self.X1, W) + b
              else: #Matmul: multiplication of two arrays summed up with its bias.
                  encoded1 = tf.matmul(encoded1, W) + b
              #Relu activation function on the encoded representation of the attribute.
        ...
        if proximity == 'second-order':
            #feature 1
            #Going to create context weights and biases.
            #Only shape is different here, don't know why.
            for i in range(1, len(sizes1)):
    ...
    #KL-divergence is used as the dissimilarirty measure. This is calculated based on the created embeddings.
    #Based on the cases we're going to gather the type of embedding and intermediate node representation.
    ...
    def energy_kl(self, u_i, u_j, proximity, node_type1, node_type2):
        ...
        #Gather the embeddings of mu and sigma of both nodes, based on the type of nodes that they are - which are the type of POI.
